...
**Demo.** The landing page's second button opens `/demo`, which starts a guest session without an account. The route mints a demo organizer id, stores it in a signed cookie, creates and publishes the symposium event from the seed in `src/demo/seed.ts` under that id, and opens the event's dashboard with `?demo=1`. A visitor whose cookie already names a demo organizer lands on the same event. The demo organizer owns that one event and answers 403 for any other event and for a second create. The dashboard's snapshot carries `demo: true`, which the tour reads.
