...
**Landing.** The root route `/` shows the problem, the five-step walkthrough with one clip per step, and the three store tools. Its button opens `/events/new` for a signed-in organizer and the sign-in page for a visitor without an account and for a guest.
