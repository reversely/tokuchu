...
| Tokuchu, on the organizer's event page | `list_guests`, `set_gift_plan`, `set_gift_customization`, `get_requirements`, `request_from_attendees`, `list_missing`, `get_fulfillment_manifest`, `approve_specs` and the rest of the organizer list (23 tools in managed mode; `search_gifts`, `send_to_vendor`, and `approve` leave in agent mode) | Lets an agent read the RSVP list, hand it the store's payload, and run the same steps this guide clicks through |
