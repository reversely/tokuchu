...
| 5. Approve and fill the cart | the organizer on the Attendees tab, then Tokuchu's server in a headless store page | `POST /api/events/{id}/gifts/{giftId}/approve-specs`, then `add_customized_to_cart` | Tokuchu's server, then the store's page | HTTP from the Attendees tab's button, then WebMCP `executeTool` in the store page; the job runs after the approve response and posts its steps into the gift's thread |
