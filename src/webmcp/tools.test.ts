...
describe("the tool definitions", () => {
  ...
  it("keeps the money-spending tools away from vendor scope", () => {
    ...
    for (const name of ["get_manifest", "get_fulfillment_manifest", "get_changes", "post_update", "post_procurement_update", "get_updates", "count_by", "get_summary"]) expect(tool(name).scopes).toContain("vendor");
...
describe("the submit_rsvp schema", () => {
  it("carries each question's constraints as JSON Schema and requires the ones needed when going", () => {
    ...
    expect(schema).toEqual({
      ...
      properties: {
        ...
        email: { type: "string", description: expect.any(String) },
      ...
      required: ["display_name", "status", "email", "name_on_map", "variant_size"],
  ...
  it("gives a date its format, a patterned text its pattern, and skips a choice with no options", () => {
    ...
    expect(Object.keys(properties)).toEqual(["display_name", "status", "email", "guest_id", "map_time", "map_date"]);
    expect(required).toEqual(["display_name", "status", "email", "map_time", "map_date"]);
  });
