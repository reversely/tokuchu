...
describe("a gift on a print-shop design", () => {
  ...
  it("a failed forward keeps the reply and writes a question from Tokuchu instead of an error", async () => {
    const shop = fakeShop();
    const { event, gift } = seed();
    await sendGift(event.id, gift.id, shop.deps, BUYER);
    ...
    expect(updatesFor(event.id, gift.id).map((u) => [u.caller, u.kind])).toEqual([["organizer", "reply"], ["tokuchu", "question"]]);
