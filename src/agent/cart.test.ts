...
describe("the cart from send to order", () => {
  ...
  it("pollOrder writes the order's status as an update from Tokuchu and repeats nothing", async () => {
    const shop = fakeShop();
    const { event, gift } = seed();
    ...
    expect(first).toMatchObject({ caller: "tokuchu", kind: "confirmed", reference: ORDER_ID, gift_id: gift.id });
    expect(await pollOrder(event.id, gift.id, shop.deps)).toBeNull();
