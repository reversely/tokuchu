...
/** Reads the order and writes its status to the thread when it moved on since Tokuchu's last update. */
export async function pollOrder(eventId: string, giftId: string, deps: CartDeps): Promise<VendorUpdate | null> {
  ...
  const last = updatesFor(eventId, giftId).filter((u) => u.caller === TOKUCHU_CALLER).at(-1);
  if (last?.kind === kind) return null;
  return postUpdate(eventId, giftId, TOKUCHU_CALLER, { kind, text: `Order ${order.id}: ${order.status ?? kind}.`, reference: order.id });
}
