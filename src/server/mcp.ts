...
async function searchOperation(eventId: string, args: ToolArgs): Promise<unknown> {
  ...
  const res = await mod.POST(new Request("http://tokuchu.local/search", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ card: args.card, sentence: args.sentence }) }), { params: Promise.resolve({ id: eventId }) });
