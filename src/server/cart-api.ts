 ...
 */
import { z } from "zod";
import { approveGift, CartStateError, TOKUCHU_CALLER, liveDeps, lockAndCheckout, lockIsDue, pollOrder, refreshCart, sendGift, syncGift, type CartDeps, type ShortLine } from "../agent/cart";
 ...
 * forward becomes a thread question from Tokuchu, which the Overview surfaces as a follow-up until
...
export async function forwardOrganizerReply(eventId: string, giftId: string, update: VendorUpdate): Promise<void> {
  ...
  try {
    ...
    postUpdate(eventId, giftId, TOKUCHU_CALLER, { kind: "question", text: "The reply did not reach the shop." });
  }
}
