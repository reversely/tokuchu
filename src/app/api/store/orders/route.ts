 ...
 * tool calls it with `checkout_url` or `cart_token` or `order_name` and the route answers the order
...
export async function GET(request: Request) {
  ...
  try {
    const state = await lookupOrder({ checkout_url: params.get("checkout_url"), cart_token: params.get("cart_token"), order_name: params.get("order_name") });
