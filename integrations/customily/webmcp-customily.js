 ...
 * `add_customized_to_cart`, so a browser agent can read a product's customization fields and add
 * configured lines through Shopify's cart endpoint from any page on the shop's origin. The three
 * older batch tools (`get_personalization_schema`, `validate_personalized_batch`,
 * `create_personalized_batch`) stay registered until issue #12 removes them once the app stops
 * calling them. Every DOM selector lives in PRODUCT_ADAPTERS, keyed by the numeric Shopify product
 * id, so the tool bodies stay generic and a new product needs only a new adapter entry.
 *
...
(function () {
  ...
  const PRODUCT_ADAPTERS = {
    "10242071789817": {
      ...
      // The fields get_customization answers when the page exposes no metafield. Each label is the
      // line item property name Customily reads. The star map widget names no property for its
      // time control, so the time label follows the storefront's own location and date labels.
      customization: [
  ...
  function register() {
    ...
    for (const tool of TOOLS.concat(CUSTOMIZATION_TOOLS)) {
