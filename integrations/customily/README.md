...
4. The page needs `document.modelContext`. A browser with native WebMCP support provides it;
   otherwise load the polyfill (`src/webmcp/polyfill.js` in this repo) from the theme's
...
`scripts/personalize-agent.ts` reads a gift's manifest from Tokuchu's tokenized MCP endpoint
...
`tests/customily-live.spec.ts` smoke-tests the adapter against the live storefront. It
