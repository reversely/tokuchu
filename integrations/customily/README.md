...
`webmcp-customily.js` registers three tools on `document.modelContext` on every page of the store, so an agent can read a product's customization fields, add configured lines to the cart through Shopify's cart endpoint, and read what became of the order after checkout. The tools make no DOM writes. Each supported product carries one entry in the `PRODUCT_ADAPTERS` map at the top of the file, keyed by the numeric Shopify product id, so supporting a new product means adding one entry or one product metafield.
...
Takes `{ checkout_url?, cart_token?, order_name? }`; with none of the three it reads this session's cart token from `/cart.js`. A theme cannot read orders, so the tool calls the order-status route Tokuchu serves for the store, `GET /api/store/orders?checkout_url=...` (or `cart_token` or `order_name`), which looks the order up through the store's Admin API and answers with CORS headers for the store's origin alone. The default route is `ORDER_STATUS_URL` at the top of the file; a theme overrides it with `window.__tokuchuOrderStatusUrl` or a `<meta name="tokuchu-order-status-url" content="...">` tag. The answer is `{ status: "ordered", order_id, order_name, created_at, updated_at, financial_status, fulfillment_status, line_items, fulfillments }`, where each line item carries `title`, `variant_id`, `quantity`, `recipient_ref` (from the `_tokuchu_recipient` property `add_customized_to_cart` wrote) and `values` keyed by the property labels Customily reads (such as `Text 2`), and each fulfilment carries `status`, `updated_at`, and `tracking` with `company`, `number`, and `url`. A checkout with no order yet answers `{ status: "not_ordered" }`.
