/**
 * Favor vendor discovery: finds party-favor merchants the way Tokuchu's agent will.
 *
 ...
 *
 * Run from the repo root: npx tsx spikes/favor-vendors/discover.mts
...
const DIR = process.env.SPIKE_DIR ?? "spikes/favor-vendors";
...
for (const q of queries) {
  ...
  for (const product of list) {
    ...
    // For Tokuchu they matter because a dietary or size split can only be expressed as a variant.
...
for (const s of [...sellers.values()].sort((a, b) => b.types.size - a.types.size || b.products - a.products)) {
  ...
  // for the cart and checkout steps Tokuchu needs later; anything else is the HTTP status we got.
