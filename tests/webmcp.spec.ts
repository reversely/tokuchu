...
test("the dashboard registers the organizer's tools; get_summary reads the seeded counts; a foreign write is an error", async ({ page, request }) => {
  ...
  expect(names).toEqual(["approve", "approve_specs", "count_by", "follow_up", "get_changes", "get_fulfillment_manifest", "get_guest", "get_manifest", "get_requirements", "get_summary", "get_updates", "list_guests", "list_missing", "post_update", "request_from_attendees", "search_gifts", "send_to_vendor", "set_gift_plan", "set_personalization_mapping"]);
