/**
 * Tokuchu's tools through Chrome's polyfill (PRD Section 12): Playwright's Chromium has no native
...
test("the dashboard registers the organizer's tools; get_summary reads the seeded counts; a foreign write is an error", async ({ page, request }) => {
  ...
  expect(names).toEqual(["acknowledge_changes", "approve", "approve_specs", "count_by", "follow_up", "get_changes", "get_fulfillment_manifest", "get_guest", "get_manifest", "get_procurement", "get_requirements", "get_summary", "get_updates", "import_guests", "list_guests", "list_missing", "load_sample_attendees", "post_procurement_update", "post_update", "request_from_attendees", "search_gifts", "send_to_vendor", "set_gift_customization", "set_gift_plan", "set_personalization_mapping"]);
