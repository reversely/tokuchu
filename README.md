...
The app deploys on Render as a Web Service built from the Dockerfile, with Render Postgres holding one JSON document per event and the sign-in tables. Sign-in uses a magic link sent through Resend. The environment names are listed in `.env.example`.
