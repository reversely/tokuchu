...
`TOKUCHU_STATIC=1` runs Tokuchu as a records app for an agent that lives in the browser or in a terminal with a browser tool. The server makes no catalog search and opens no store page of its own: the agent reads a product's `get_customization` on the store's page and hands the payload to Tokuchu through `set_gift_customization`, and after `approve_specs` it takes the `cart_items` that `get_fulfillment_manifest` returns to the store's page and calls `add_customized_to_cart` there. The Guest Experience tab shows a handoff card per gift with the store URL, the product handle, the gift id, and the next tool on each side, every page carries an Agent notes block and a `tokuchu-agent-task` meta tag with the order of operations, and the assistant, the schedules, and the proactive posts are off. `docs/agent-playbook.md` is the prompt and the steps for an agent; `npm run test:static` plays the agent across both pages with the polyfill against a static server on port 3114 (`LIVE_CUSTOMILY=1` adds the live store half), and `npm run agent-playbook` runs the playbook as a script and records a video.
